...
class BuffTemps(Enum):
    ...
    chihuan = BuffTemp('chihuan', BuffTypes.Harm, True, True, False,
                       [[ModifierEffect(RS.always_true, {ma.physical_protect_range: 1, ma.move_range: -1})],
                        [ModifierEffect(RS.always_true,
                                        {ma.physical_protect_range: 1, ma.magic_protect_range: 1, ma.move_range: -2})],
                        [ModifierEffect(RS.always_true,
                                        {ma.physical_protect_range: 1, ma.magic_protect_range: 1, ma.move_range: -3})]])

    # 封劲	有害	可驱散	可扩散	可偷取	主动绝学射程-1
    fengjing = BuffTemp('fengjing', BuffTypes.Harm, True, True, True,
    ...
    youshuang = BuffTemp('youshuang', BuffTypes.Others, False, False, False, [
        ModifierEffect(RS.always_true,
                       {ma.magic_damage_reduction_percentage: 20, ma.physical_damage_reduction_percentage: 20})],
                         [
                             EventListener(EventTypes.damage_end, 1, RS.always_true,
                                           partial(Effects.add_fixed_damage_with_attack_and_defense, multiplier=0.3,
                                                   is_magic=False)),
                             EventListener(EventTypes.damage_end, 2, RS.always_true,
                                           partial(Effects.add_target_harm_buffs, buff_temp=['chihuan'], duration=2))])

    # 禁闭 其他 不可驱散	不可扩散	不可偷取	无法行动，且在对战中无法进行反击。行动结束时，对周围2格内所有友方施加1个随机「有害状态」（不可驱散，队友行动结束若处于自身1格范围移除）
    jinbi = BuffTemp('jinbi', BuffTypes.Others, False, False, False,
                     [ModifierEffect(RS.always_true,
    ...
    sanmei_zhenhuo = BuffTemp('sanmei_zhenhuo', BuffTypes.Harm, True, False, False,
                              [ModifierEffect(RS.always_true, {ma.magic_defense_percentage: -15})],
                              [EventListener(EventTypes.action_end, 1,
                                             RS.always_true,
                                             partial(Effects.take_magic_damage_of_buff_caster, multiplier=0.5))])

    # 中毒	有害	可驱散	可扩散	可偷取	行动结束时，损失10%气血，若每多移动1格，则额外损失5%气血（最多15%）
    zhongdu = BuffTemp('zhongdu', BuffTypes.Harm, True, True, True, [], [
        EventListener(EventTypes.action_end, 1, RS.always_true,
                      partial(Effects.take_fixed_damage_by_percentage, percentage=0.1)),
        EventListener(EventTypes.action_end, 1, RS.always_true,
                      partial(Effects.take_fixed_damage_by_percentage_per_each_move, percentage=0.05))])
    ...
    luanshen = BuffTemp('luanshen', BuffTypes.Harm, True, True, False,
                        [[ModifierEffect(RS.always_true,
                         ...
                         [ModifierEffect(RS.always_true,
                                         {ma.physical_damage_percentage: -0, ma.magic_damage_percentage: -20})]])
    ...
    xianqu = BuffTemp('xianqu', BuffTypes.Others, False, False, False, [
        ModifierEffect(RS.always_true, {ma.magic_damage_reduction_percentage: 20, ma.magic_protect_range: 2})], [
                          EventListener(EventTypes.battle_end, 3, RS.always_true,
    ...
    bingzhili = BuffTemp('bingzhili', BuffTypes.Others, False, False, False,
                         [ModifierEffect(partial(RS.BuffChecks.target_has_certain_buff, 'chihuan'),
                                         {ma.physical_damage_percentage: 20})],
                         [])

    # 冰劫	其他	不可驱散	不可扩散	不可偷取	主动移动时，若移动距离小于等于1格，行动结束时，将「冰劫」替换为「晕眩」，持续1回合。
    bingjie = BuffTemp('bingjie', BuffTypes.Others, False, False, False, [], [
        EventListener(EventTypes.move_end, 1, partial(RS.move_less_or_equal_than, 1),
    ...
    cigu = BuffTemp('cigu', BuffTypes.Benefit, True, True, True,
    ...
    guyuan = BuffTemp('guyuan', BuffTypes.Benefit, True, True, True,
                      [[ModifierEffect(RS.always_true, {ma.magic_damage_reduction_percentage: 10})],
                       [ModifierEffect(RS.always_true, {ma.magic_damage_reduction_percentage: 20})],
                       [ModifierEffect(RS.always_true, {ma.magic_damage_reduction_percentage: 25})]], [])

    # 失智I	有害	可驱散	可扩散	可偷取	法术免伤-10%
    shizhi = BuffTemp('shizhi', BuffTypes.Harm, True, True, True,
                      [[ModifierEffect(RS.always_true, {ma.magic_damage_reduction_percentage: -10})],
                       [ModifierEffect(RS.always_true, {ma.magic_damage_reduction_percentage: -20})],
                       [ModifierEffect(RS.always_true, {ma.magic_damage_reduction_percentage: -25})]], [])

    # 御魔I	有益	可驱散	可扩散	可偷取	法防+30%
    yumou = BuffTemp('yumou', BuffTypes.Benefit, True, True, True,
    ...
    piruo = BuffTemp('piruo', BuffTypes.Harm, True, True, False,
                     [[ModifierEffect(RS.always_true, {ma.attack_percentage: -20, ma.magic_attack_percentage: -20})],
                      [ModifierEffect(RS.always_true, {ma.attack_percentage: -25, ma.magic_attack_percentage: -25})]],
                     [])
    ...
    shenxing = BuffTemp('shenxing', BuffTypes.Others, False, False, False,
                        [[ModifierEffect(RS.always_true, {ma.move_range: 1})],
                         [ModifierEffect(RS.always_true, {ma.move_range: 2})],
                         [ModifierEffect(RS.always_true, {ma.move_range: 3})]], [
                            EventListener(EventTypes.critical_damage_end, 1, RS.always_true,
                                          partial(Effects.reduce_skill_cooldown, 1))])

    # 芬芳I	有益	可驱散	可扩散	可偷取	治疗效果提高20%
    fenfang = BuffTemp('fenfang', BuffTypes.Benefit, True, True, True,
    ...
    xunji = BuffTemp('xunji', BuffTypes.Benefit, True, True, True,
                     [[ModifierEffect(RS.always_true, {ma.move_range: 1})],
                      [ModifierEffect(RS.always_true, {ma.move_range: 2})],
                      [ModifierEffect(RS.always_true, {ma.move_range: 3})]], [])
    ...
    duozui = BuffTemp('duozui', BuffTypes.Harm, False, False, False,
                      [ModifierEffect(partial(RS.attacked_by_caster), {ma.magic_damage_reduction_percentage: -10})], [])
